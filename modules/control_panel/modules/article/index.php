...
namespace MODULES\article{
    ...
    function save($data){
        ...
        if($row[0] == 0){
            $curdate = date('Y-m-d');
            $result = $dblink->query("INSERT INTO {$dbprefix}".table." (title,content,date) VALUES ('{$_REQUEST['title']}','{$_REQUEST['content']}','{$curdate}')");
        }else{
            $result = $dblink->query("UPDATE {$dbprefix}".table." SET title='{$_REQUEST['title']}', content='{$_REQUEST['content']}' WHERE id={$_REQUEST['id']}");
        }
